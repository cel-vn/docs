...
export default function PublicDatabaseViewer() {
  const [data, setData] = useState<PublicDatabaseData | null>(null);
  ...
  return (
    <div className="min-h-screen bg-fd-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <span className="ml-4 text-fd-muted-foreground">Public Database View</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-fd-foreground">Database Statistics</h1>
          ...
          <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-md">
            <p className="text-sm text-blue-800">
              🔒 <strong>Privacy Note:</strong> {data.note}
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-fd-card p-6 rounded-lg border border-fd-border">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-fd-muted-foreground">Recent (24h):</span>
                  <span className="font-medium text-purple-600">{data.stats.recentOTPs}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'users' && (
          <div className="bg-fd-card rounded-lg border border-fd-border overflow-hidden">
            <div className="px-6 py-4 border-b border-fd-border">
              <h2 className="text-lg font-semibold text-fd-foreground">All Users (Privacy Protected)</h2>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-fd-border">
                ...
                <tbody className="bg-fd-card divide-y divide-fd-border">
                  {data.users.map((user) => (
                    <tr key={user.id}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-fd-muted-foreground">
                        {user.id}
                      </td>
                  ...
                  <tbody className="bg-fd-card divide-y divide-fd-border">
                    {data.otps.map((otp) => (
                      <tr key={otp.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full border ${getStatusBadgeColor(otp.status)}`}>
                            {otp.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-fd-foreground">
